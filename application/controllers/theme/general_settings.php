...
class General_settings extends Admin_controller
{
    ...
    public function save_site_info()
    {
        ...
        redirect('theme/general_settings', 'refresh');
    }

    // save print baner
    public function save_print_banner()
    {
        if (!empty($_FILES["print_banner"]["name"]) && $_FILES["print_banner"]["name"] != "") {

            // delete file
            if (!empty($_POST['old_print_banner'])) {
                if (file_exists($_POST['old_print_banner'])) {
                    unlink($_POST['old_print_banner']);
                }
            }

            // upload file
            $path = file_upload('print_banner', 'upload/config');

            // input data
            $data = [
                'config_key'   => 'print_banner',
                'config_value' => $path
            ];

            // save or update data
            if (check_exists('tbl_config', ['config_key' => 'print_banner'])) {
                $msg = array(
                    "title" => "success",
                    "emit"  => "Print banner updated successfully",
                    "btn"   => true
                );
                save_data('tbl_config', $data, ['config_key' => 'print_banner']);
            } else {
