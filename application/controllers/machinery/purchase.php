...
class Purchase extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('components/machinery/create', $this->data);
    ...
    public function store()
    {
        if (isset($_POST['save'])) {
            ...
            $options = array(
                'title' => 'success',
                'emit'  => 'Machinery purchase successfully completed.',
                'btn'   => true
            );

            $this->session->set_flashdata("confirmation", message('success', $options));
        }
        redirect("machinery/purchase", "refresh");
    }
    ...
    private function handelStock($index)
    {
        ...
        // set the quantity
        $quantity = (!empty($record)) ? ($record[0]->quantity + $_POST['quantity'][$index]) : $_POST['quantity'][$index];
    ...
    public function show_purchase()
    {
        ...
        $where = ['saprecords.status' => 'purchase', 'saprecords.trash' => 0, 'sapitems.trash' => 0, 'materials.type' => 'raw', 'materials.raw_type' => 'machinery'];
        ...
        $this->load->view($this->data['privilege'] . '/includes/headermenu', $this->data);
        $this->load->view('components/machinery/nav', $this->data);
        $this->load->view('components/machinery/all-purchase', $this->data);
        $this->load->view($this->data['privilege'] . '/includes/footer');
    }
    ...
    public function preview_voucher()
    {
        ...
        $this->load->view($this->data['privilege'] . '/includes/headermenu', $this->data);
        $this->load->view('components/machinery/nav', $this->data);
        $this->load->view('components/machinery/preview-voucher', $this->data);
        $this->load->view($this->data['privilege'] . '/includes/footer');
    }
    ...
    public function delete_purchase()
    {
        ...
        redirect('machinery/purchase/show_purchase', 'refresh');
    }
    ...
    public function item_wise()
    {
        ...
        $where                 = array('type' => "raw", 'raw_type' => 'machinery');
        $this->data['rawMats'] = $this->action->read('materials', $where);

        $where = [
            'materials.type'     => 'raw',
            'materials.raw_type' => 'machinery',
        ...
        $this->load->view($this->data['privilege'] . '/includes/headermenu', $this->data);
        $this->load->view('components/machinery/nav', $this->data);
        $this->load->view('components/machinery/item-wise', $this->data);
        $this->load->view($this->data['privilege'] . '/includes/footer');
    }
